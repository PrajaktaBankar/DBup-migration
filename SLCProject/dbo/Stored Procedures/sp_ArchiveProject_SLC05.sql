...
CREATE PROCEDURE [dbo].[sp_ArchiveProject_SLC05]
(
	@SLC_CustomerId					INT
...
BEGIN
	...
	BEGIN TRY
		...
		--INNER JOIN [SLCSERVER05].[SLCProject].[dbo].[Staging_ProjectSection] PSC WITH (NOLOCK)
		--	ON S.Sectionid = PSC.SectionId AND S.ProjectId = PSC.ProjectId AND S.CustomerId = PSC.CustomerId
		WHERE S.ProjectId = @SLC_ProdProjectId AND S.CustomerId = @SLC_CustomerId
		...
		FROM [SLCSERVER05].[SLCProject].[dbo].[ProjectImage] I WITH (NOLOCK) WHERE I.CustomerId = @SLC_CustomerId
		...
		SELECT @RowExists = COUNT(1) FROM [SLCSERVER05].[SLCProject].[dbo].[MaterialSection] S WITH (NOLOCK)
		WHERE S.ProjectId = @SLC_ProdProjectId AND S.CustomerId = @SLC_CustomerId
		...
		UPDATE P SET P.IsDeleted = 1, P.IsPermanentDeleted = 1 FROM [SLCSERVER05].[SLCProject].[dbo].[Project] P WITH (NOLOCK)
	...
	BEGIN CATCH
		...
		*************************************/
		...
		--Update Project Deleted and Permanent Deleted to True for the project from Archive Server as we have unarchived the project already
		UPDATE P SET P.IsArchived = 0 FROM [SLCSERVER05].[SLCProject].[dbo].[Project] P WITH (NOLOCK)
		WHERE P.ProjectId = @SLC_ProdProjectId AND P.CustomerId = @SLC_CustomerId
