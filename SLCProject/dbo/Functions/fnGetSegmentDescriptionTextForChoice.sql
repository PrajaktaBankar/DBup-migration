...
BEGIN
	...
	DECLARE @ProjectId int,@origin nvarchar(2),@sectionId int,@segmentId int,@mSegmentId int,@mSegmentStatusId INT,@CustomerId INT
	DECLARE @Description NVARCHAR(1024),@sourceTag VARCHAR(18)
	DECLARE @segmentDescription NVARCHAR(max)=''

	SELECT 
	@ProjectId=ProjectId,@origin=SegmentOrigin,@sectionId=SectionId,
	@segmentId=SegmentId,@mSegmentId=mSegmentId,@mSegmentStatusId=mSegmentStatusId,@CustomerId=CustomerId
  ...
  --All Choices
   ...
   BEGIN
	   ...
	   BEGIN
			...
			INNER JOIN [SelectedChoiceOption] sco with (nolock)
			ON sco.ProjectId = sc.ProjectId AND sco.CustomerId = sc.CustomerId AND sco.SectionId = sc.SectionId
			AND sco.SegmentChoiceCode = sc.SegmentChoiceCode
			and sco.ChoiceOptionCode=co.ChoiceOptionCode
			WHERE sc.ProjectId = @ProjectId and sc.CustomerId = @CustomerId AND sc.SectionId=@sectionId and  
			...
			BEGIN
				...
				AND SegmentChoiceCode=@choiceCode AND SegmentStatusId=@segmentStatusId
