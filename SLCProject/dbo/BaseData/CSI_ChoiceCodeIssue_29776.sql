...
--Customer Support 29776: CH# in section
...
INNER JOIN ProjectSegment PS
	ON PS.SegmentStatusId = PSSV.SegmentStatusId
		AND PS.SegmentId = PSSV.SegmentId
		AND PS.ProjectId = PSSV.ProjectId
WHERE PSSV.ProjectId = @ProjectId
AND ISNULL(PSSV.IsDeleted, 0) = 0
AND PSSV.SegmentOrigin = 'U'
...
SELECT
   ...
   ,CCIS.ChoiceCode
   ,CCIS.CustomerId
   ,CCIS.SegmentStatusId
   ,CCIS.SegmentId
   ,CCIS.ModifiedBy INTO #TempProjectSegmentChoice1
FROM #ChoiceCodesInSegmnetDesc CCIS
LEFT OUTER JOIN #TempProjectSegmentChoice TPSCO
	ON TPSCO.SegmentChoiceCode = CCIS.ChoiceCode
...
SELECT
	SLCMCO.* INTO #TempProjectChoiceOption1
...
LEFT OUTER JOIN ProjectChoiceOption PCO
	ON PCO.ChoiceOptionCode = SLCMCO.ChoiceOptionCode
		AND tblins.ProjectId = PCO.ProjectId
		AND tblins.SectionId = PCO.SectionId
WHERE SC.SegmentChoiceId = SLCMCO.SegmentChoiceId
...
SELECT
   ...
   ,CCIS.SegmentStatusId
   ,CCIS.SegmentId
   ,CCIS.ModifiedBy INTO #TempProjectSelectedChoiceOption1
FROM #ChoiceCodesInSegmnetDesc CCIS
...
INSERT INTO ProjectChoiceOption
	...
	INNER JOIN SLCMaster..SegmentChoice SLCSC
		ON tblins.ChoiceCode = SLCSC.SegmentChoiceCode
	INNER JOIN #TempProjectChoiceOption1 SC WITH (NOLOCK)
		ON SC.SegmentChoiceId = SLCSC.SegmentChoiceId
