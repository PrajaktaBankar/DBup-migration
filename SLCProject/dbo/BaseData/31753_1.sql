USE SLCProject
GO
  ---Customer Support 31753: Choices Duplicated in Section
  ---Execute on Server 4
 
  DROP TABLE IF EXISTS #duplicateProjectSegmentChoice

  DECLARE @ProjectId int=692
...
---filter duplicate data from ProjectChoiceOption and insert it into temp table
   ...
   FROM ProjectChoiceOption WITH(NOLOCK) WHERE ProjectId=@ProjectId
   )AS A WHERE A.rowno>1
   
   ---Delete duplicate data from selectedChoiceOPtion table
   ...
   --delete duplicate data from ProjectChoiceOption table
   DELETE A   FROM (
   SELECT  * ,ROW_NUMBER()OVER(PARTITION BY SegmentChoiceId,SortOrder,ProjectId,SectionId,CustomerId ORDER BY ChoiceOptionId)as rowno 
   FROM ProjectChoiceOption WITH(NOLOCK)  WHERE ProjectId=@ProjectId
   )AS A WHERE A.rowno>1
  ...
  UPDATE pco SET pco.OptionJson=slcmco.OptionJson FROM 
  SLCMaster..ChoiceOption slcmco WITH(NOLOCK) INNER JOIN 
  ProjectChoiceOption pco WITH(NOLOCK) ON pco.ChoiceOptionCode=slcmco.ChoiceOptionCode  
  WHERE pco.ChoiceOptionCode in (
  295957,
  295958,
  295959,
  295960,
  295961) and slcmco.SegmentChoiceId=121344 AND pco.ProjectId=@ProjectId