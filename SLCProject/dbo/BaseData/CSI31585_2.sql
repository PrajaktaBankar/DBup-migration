/*
Customer Support 31585: {CH#} ISSUE in "One Gas" project
...
-----------------------------DELETIONS---------------------------------
   ...
   ---IF SelectedChoiceOption record is deleted and it is available in ProjectSegmentChoice and ProjectChoiceOption then insert it into SelectedChoiceOption
 ...
 INNER JOIN SLCMaster..SelectedChoiceOption smsco WITH(NOLOCK)  ON smsco.ChoiceOptionCode=pco.ChoiceOptionCode
 ...
 DELETE A FROM (
SELECT *,ROW_NUMBER()OVER(PARTITION BY SegmentChoiceCode	,ChoiceOptionCode	,		SectionId, ProjectId	,CustomerId ORDER BY SelectedChoiceOptionId)as rowno
 FROM SelectedChoiceOption WITH(NOLOCK)  WHERE   ProjectId=@ProjectId and   ChoiceOptionSource='U'
 )as A WHERE A.rowno>1

 DELETE A  FROM (
 SELECT *,ROW_NUMBER()OVER(PARTITION BY SegmentChoiceId,SortOrder,ProjectId	,SectionId	,CustomerId	,ChoiceOptionCode ORDER BY ChoiceOptionId)as rowno
 FROM ProjectChoiceOption WITH(NOLOCK)  WHERE   ProjectId=@ProjectId  AND ChoiceOptionSource='U'
 )as A WHERE A.rowno>1