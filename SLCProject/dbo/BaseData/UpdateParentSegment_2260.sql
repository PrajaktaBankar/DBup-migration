...
DECLARE @ProjectId INT = 2260
DECLARE @SectionId INT = 2529114

select ROW_NUMBER() over(order by mSegmentId) as RowId,mSegmentId,COUNT(SegmentStatusId) AS StatusCount 
into #duplicateSegments from [dbo].[ProjectSegmentStatus] ps 
where ps.ProjectId=@ProjectId and ps.SectionId=@SectionId --and mSegmentId=711384
GROUP BY mSegmentId
...
begin
	...
	begin
		...
		where t.ParentSegmentStatusId=(select x.SegmentStatusId from #dProjectSegmentStatus x where 
		x.SegmentStatusId in(t.ParentSegmentStatusId) and isnull(x.IsDeleted,0)=0)
