...
INSERT INTO #tempProjectId (ProjectId  ,RowNo  )
 ...
 ROW_NUMBER () over(PARTITION BY  CustomerId ORDER BY  CustomerId)as RowNo
  from 
  Project WITH(NOLOCK) WHERE  CustomerId=1596 and ISNULL( IsPermanentDeleted,0)=0
 ...
 DECLARE @ProjectId INT =  (select top 1 ProjectId  from #tempProjectId WHERE RowNo=@ProjectRowCount);
